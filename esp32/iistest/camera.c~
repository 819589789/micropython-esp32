...
static xTaskHandle  xhttpserver = NULL;
/*
...
static esp_err_t camerarun()
{
	...
	//printf("Waiting for frame OKOKO\n");
...
static void http_server_netconn_serve(struct netconn *conn)
{
            ...
            if (buf[5] == 's') {
                ...
                if (s_pixel_format == CAMERA_PF_JPEG) {
                    ...
                    if (memcmp(&buf[5], "pgm", 3) == 0) {
                        ...
                        netconn_write(conn, pgm_header, strlen(pgm_header), NETCONN_NOCOPY);
                    }
                }
                ...
                if (err != ESP_OK) {
                    ...
                    ESP_LOGD(TAG, "Done");
                    //Send jpeg
					ESP_LOGE(TAG, "begin netconn--Free heap: %u", xPortGetFreeHeapSize());
					uint8_t *getfb;
					size_t getlen = 0;
					size_t i = 0;
					getfb  = camera_get_fb();
					getlen = camera_get_data_size();
					//printf("getlen=%d\n",getlen);
					struct timeval tv_start;
					gettimeofday(&tv_start, NULL);
					if(getlen == 76800){
						while(i<76800){
							err = netconn_write(conn, getfb+i, 400,
                        	NETCONN_NOCOPY);
							i += 400;
							if(err != ESP_OK){
								printf("err!=ESP_OK=%d\n",err);
						...
						while(i<38400){
							...
							err = netconn_write(conn, getfb, getlen,
                        	NETCONN_NOCOPY);
					}
					struct timeval tv_end;
					gettimeofday(&tv_end, NULL);
	...
	//printf("colse netconn_close\n");
    /* Close the connection (server closes in HTTP) */
    err = netconn_close(conn);
	//printf("netconn_close=%d\n",err);
...
static void http_server(void *pvParameters)
{
    ...
    do {
        ...
        if (err == ERR_OK) {
			...
			//printf("netconn_delete err=%d\n",err);
        }else{
			//printf("netconn_accept---err\n");
		}
		vTaskDelay(1000/ portTICK_PERIOD_MS);
    } while (err == ERR_OK);
	//printf("http_server task over\n");
    netconn_close(conn);
    netconn_delete(conn);
	free(conn);
	xhttpserver = NULL;
	vTaskDelete(xhttpserver);
}
...
*/
    printf("Allocating frame buffer (%d bytes)\n", s_state->fb_size);
    if(pix_format == 0){
        //s_state->fb = (uint8_t*) calloc(10, (s_state->fb_size)/10);
		s_state->fb = (uint8_t*) malloc(s_state->fb_size);
        if(s_state->fb == NULL){
...
*/
		...
		//printf("camera_init=====76800\n");
        s_state->fb = (uint8_t*) calloc(1, s_state->fb_size);
        if(s_state->fb == NULL){
            ESP_LOGE(TAG, "Failed to allocate frame buffer");
            err = ESP_ERR_NO_MEM;
            goto fail;
	...
	//printf("begin xTaskCreatePinnedToCore\n");
	...
	gpio_intr_disable((gpio_num_t)s_state->config.pin_vsync);
	vTaskDelay(10);
...
static void get_bmp(const char *pictureFilename)
{
	...
	struct stat sb;
	char *dirname = get_dir_name((char *)pictureFilename);
	if(stat(dirname, &sb)){
...
esp_err_t camera_run(const char *pictureFilename)
{
    ...
    gettimeofday(&tv_end, NULL);
    int time_ms = (tv_end.tv_sec - tv_start.tv_sec) * 1000 + (tv_end.tv_usec - tv_start.tv_usec) / 1000;
    printf("Frame %d done in %d ms\n", s_state->frame_count, time_ms);
    s_state->frame_count++;
    return ESP_OK;
}
...
static void i2s_init()
{
	...
	if(err != ESP_OK){
		printf("esp_intr_alloc=======err =0x%x\n",err);
	}
...
static void i2s_run()
{
	...
	esp_err_t err;
	err = esp_intr_disable(s_state->i2s_intr_handle);
/*
	if(err != ESP_OK){
		printf("esp_intr_disable err=0x%x\n",err);
	}
*/
    i2s_conf_reset();
    ...
    err = esp_intr_enable(s_state->i2s_intr_handle);
	//printf("esp_intr_enable err=0x%02x\n",err);
...
/*
	...
	//uint8_t *buf1;
	//memcpy(buf1,buf,end*4);
	photodataEnqueue(buf,2);
*/
}
...
void sendtonet(void)
{
	...
	xTaskCreate(&http_server, "http_server", 4096, NULL, 5, &xhttpserver);
}
